...
async def send_broadcast_with_media_group(photo_paths, message_text):
    ...
    photo_path1 = 'bot/images/img_31.png'
    ...
    message_template = (
        "{name}, привет подружка! 🥰\n\
    \n\
Скоро Новый Год, ты тоже уже ломаешь голову над подарками?\n\
Я хотела поделиться идей подарка, мне попалось на глаза нижнее белье от MYK - это просто находка🙌🏽😃\n\
    \n\
Во-первых, это очень красивый и качественный подарок\n\
А во-вторых, он несет в себе такой глубокий смысл - внимание и заботу🥰\n\
    \n\
Кстати, у них сейчас акция, так что можно еще и сэкономить\n\
    \n\
Не откладывай, чтобы успеть оформить заказ до праздников!"
)
