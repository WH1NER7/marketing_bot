...
async def send_broadcast_with_media_group(photo_paths, message_text):
    ...
    photo_path1 = 'bot/images/img_73.png'
    ...
    message_template = (
        "<b>{name}, определилась, куда поедешь в отпуск?</b>\n\
    \n\
Турция, Бали, Италия или хоть речка на даче — главное, чтобы ты кайфовала на полную 😼\n\
    \n\
<b>И не забывай про самое важное - как ты будешь выглядеть там. Выбирай купальник, который хочется носить даже до вылета😁</b>"
    )
