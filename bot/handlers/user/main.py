...
async def send_broadcast_with_media_group(photo_paths, message_text):
    ...
    photo_path1 = 'bot/images/img_68.png'
    ...
    message_template = (
        "{name}, 90% мужчин не могут отвести от тебя взгляд 👀\n\
\n\
Потому что ты в белье, которое не только идеально сидит, но и излучает уверенность. <b>Это не просто ткань — это твой секретный союзник</b>💋\n\
\n\
Тот самый комплект, который станет твоим <b><u>оружием притяжения</u></b>, уже ждет тебя."
    )
