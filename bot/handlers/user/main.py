...
from bot.keyboards.inline import markup_lk, markup_competition, markup_link, faq_kb, shop_kb
...
async def start(message: Message):
    ...
    photo = types.InputFile('bot/images/start.jpg')
...
async def service(message: Message):
    photo = types.InputFile('bot/images/mashinka.jpg')
    await message.answer_photo(caption=f'Мы тщательно следим за качеством пошива наших изделий. \n\
...
async def competition(message: Message):
    photo = types.InputFile('bot/images/dev.png')
    await message.answer_photo(caption=f'Милая, скоро у тебя появится возможность выиграть новый Iphone. Включи уведомления, чтобы не пропустить новости от нас!',
                               photo=photo,
...
def register_users_handlers(dp: Dispatcher) -> None:
    ...
    dp.register_message_handler(competition, content_types=['text'], text="Конкурсы и акции")
