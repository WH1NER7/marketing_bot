...
async def send_broadcast_with_media_group(photo_paths, message_text):
    ...
    photo_path1 = 'bot/images/img_35.png'
    ...
    message_template = (
        "{name} представь, что у тебя есть <span class='tg-spoiler'>лонгслив</span> который:\n\
    \n\
Своей мягкой сеточкой нежно обнимет твоё тело, а стильный принтованный дизайн позволит тебе создавать бесконечное множество образов - от повседневных до вечерних. А всё потому, что мы трудились над нашим лонгсливом 4 месяца, и учли все твои пожелания:\n\
    \n\
- Идеальная длина для комфорта в разных образах\n\
- Проработанная строчка, обеспечивающая безупречный внешний вид\n\
- Надежная сеточка, которая не подведет даже после многократных стирок\n\
    \n\
 Погрузись в дикую природу стиля 😼"
)
