...
async def send_broadcast_with_media_group(photo_paths, message_text):
    ...
    photo_path1 = 'bot/images/img_22.png'

    blocked_users = 0
    successful_sends = 0

#
    text_with_link = "Почему так важно ощущать уверенность в себе?\n\
\n\
||Уверенность — это внутренний свет, который притягивает позитивные изменения и вдохновляет окружающих\\.||\n\
\n\
Она помогает достигать целей и наслаждаться каждым мгновением жизни\\.\n\
\n\
**Сегодня мы поделимся с вами простым способом стать увереннее\.** Повторяйте эти аффирмации с верой и осознанием:\n\
\n\
✨Я прекрасна\\.\n\
\n\
✨Я всегда сексуальна и красива\\.\n\
\n\
✨ Я достойна любви и уважения\\.\n\
\n\
✨Я всегда достойна лучшего\\.\n\
\n\
✨Я всегда уверена в себе\\!\n\
\n\
Пусть эти фразы станут вашей личной мантрой\\."
    ...
    for subscriber_id in subscribers:
        try:
            await bot.send_photo(subscriber_id, photo=types.InputFile(photo_path1), caption=text_with_link,
                                 parse_mode=types.ParseMode.MARKDOWN_V2, reply_markup=start_kb_markup)
