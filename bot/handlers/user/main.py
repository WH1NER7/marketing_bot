...
async def send_broadcast_with_media_group(photo_paths, message_text):
    ...
    photo_path1 = 'bot/images/img_54.png'
    ...
    message_template = (
        "{name}, мы отправили тебе подарок!\n\
    \n\
Мы рассказали все подробности в Telegram, но вот спойлер: теперь к определенным комплектам MissYourKiss ты получишь роскошный домашний парфюм Black Pepper от бренда NICE\n\
    \n\
Твой уютный ритуал станет еще приятнее. Заглядывай в канал за всеми деталями! 💞"
    )
