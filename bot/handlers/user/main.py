...
async def send_broadcast_with_media_group(photo_paths, message_text):
    ...
    photo_path1 = 'bot/images/IMG_8664.MOV'
    ...
    message_template = (
        "{name}, покажи всем какая ты 🔥\n\
    \n\
Без навязанных стандартов, без бесконечных «подкачаться к лету» и «идеальных форм». <b>Просто ты — красивая, уверенная и в белье, в котором тебе реально комфортно.</b>\n\
    \n\
<b><u>Ты не должна никому соответствовать.</u></b> Главное — чувствовать себя классно. И мы точно знаем, какое белье с этим поможет 💫"
    )
