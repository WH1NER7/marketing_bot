...
async def send_broadcast_with_media_group(photo_paths, message_text):
    ...
    photo_path1 = 'bot/images/img_56.png'
    ...
    message_template = (
        "<b>{name}, активировать купон?</b>\n\
    \n\
Правильное белье — это не просто комфорт, это твой личный бонус к уверенности. Когда ты знаешь, что даже под самым обычным образом скрывается что-то особенное, <u>осанка выпрямляется, взгляд становится смелее, а шаг — увереннее.</u>\n\
    \n\
Ты это чувствуешь?\n\
Тогда купон уже сработал 💋"
    )
