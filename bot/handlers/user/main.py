...
async def send_broadcast_with_media_group(photo_paths, message_text):
    ...
    message_template = (
        "<b>Комплексная борьба с отеками к лету началась! 🏃🏻‍♀️☀️</b>\n\
    \n\
Приглашённый в наш тг-канал эксперт — фитнес-тренер — провёл для нас <b>лимфодренажную тренировку</b> и <b>поделился рецептами питания с правильным балансом соли, сахара и воды<b>, чтобы отёки больше не возвращались\n\
🏃🏻‍♀️+ 🥑\n\
\n\
Переходи в наш ТГ - тут мы болтаем как подружки и делимся друг с другом чем-то полезным ❤️"
    )
