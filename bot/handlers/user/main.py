...
async def send_broadcast_with_media_group(photo_paths, message_text):
    ...
    photo_path1 = 'bot/images/img_46.png'
    ...
    message_template = (
        "{name}, мы хотим тебе сказать что-то важное\n\
\n\
Ты — воплощение красоты, силы и грации. Это и нежная женственность и внутренний свет, который зажигает сердца.\n\
\n\
Не бойтесь быть собой, проявляйте свою уникальность, ведь именно она делает вас такими прекрасными.\n\
\n\
❤️Ваша красота — это источник вдохновения и мощный двигатель"
    )
